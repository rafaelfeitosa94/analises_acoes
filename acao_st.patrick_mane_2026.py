...
st.set_page_config(
    page_title="Dashboard Mané - Análise ST. Patrick",
    page_icon="🍺",
    layout="wide",
...
# Título principal
st.markdown("""
<h1 style="font-size: 2.5rem; color: #FF6666; font-weight: bold; margin-bottom: 0.5rem; text-shadow: 2px 2px 4px rgba(139,0,0,0.1); text-align: center;">🍀 DIA DE ST. PATRICK MANÉ 2026 🍻</h1>
""", unsafe_allow_html=True)
...
with col1:
    ...
    st.markdown('<h3 style="color: #FF6666;">🏆 Ranking de Faturamento ST. Patrick</h3>', unsafe_allow_html=True)
...
with col2:
    ...
    if not df_comp.empty:
        cores = ['#FF6666' if x >= 0 else '#8B0000' for x in df_comp['VARIACAO_FAT_%']]
...
with col2:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.markdown('<h3 style="color: #FF6666;">📊 Participação no Faturamento ST. Patrick</h3>', unsafe_allow_html=True)

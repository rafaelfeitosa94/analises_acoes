...
st.set_page_config(
    page_title="Dia das Mães Buteco 2026",
    page_icon="🌹",
    layout="wide",
...
# =========================================================
# COLORS - TONS DE AZUL (MARCA BUTECO)
# =========================================================

PRIMARY      = "#003366"      # Azul marinho principal
PRIMARY_DARK = "#001a33"      # Azul mais escuro
SECONDARY    = "#1a4d80"      # Azul médio
ACCENT       = "#89CFF0"      # Azul bebê/claro
...
.metric-card:hover {{ transform: translateY(-3px); border-color: rgba(0,51,102,.6); }}
.metric-card::after {{
    content: '';
    position: absolute;
    width: 120px; height: 120px;
    background: radial-gradient(circle, rgba(0,51,102,.10) 0%, transparent 70%);
    top: -40px; right: -40px;
}}
.metric-value {{
...
.rank-card.rank-1 {{ border-color: rgba(0,51,102,0.5); background: rgba(0,51,102,0.10); }}
.rank-card.rank-2 {{ border-color: rgba(26,77,128,0.35); }}
.rank-card.rank-3 {{ border-color: rgba(137,207,240,0.25); }}
...
">
    ...
    <div style="font-family:'Barlow Condensed', sans-serif;font-size:2.4rem;font-weight:800;
        color:#003366;letter-spacing:-0.5px;line-height:1.1;text-transform:uppercase;">
        🌹 Dia das Mães Buteco 2026 🌹
    </div>
...
data = {
    ...
    'LOJA': [
        'BUT FREGUESIA', 'BUT OLEGÁRIO', 'BUT ICARAÍ', 'BUT BARRA SUL',
        'BUT BOTAFOGO', 'BUT CIDADE JARDIM', 'BUT RIO 2', 'BUT MOEMA',
        'BUT COLLAB ENGENHÃO'
    ],
    'FAT_2026': [
        21263.91, 13661.76, 9703.35, 8933.84, 8772.39, 8547.43,
        7522.58, 2236.43, 563.20
    ],
    'TC_2026': [170, 168, 139, 139, 147, 136, 177, 25, 29],
    'TM_2026': [
        125.08, 81.32, 69.81, 64.27, 59.68, 62.85, 42.50, 89.46, 19.42
    ],
    'FAT_2025': [
        28519.88, 12421.40, 10824.63, 4615.97, 6498.97, 14933.81,
        5902.81, 12051.50, 1055.00
    ],
    'TC_2025': [257, 158, 169, 76, 90, 189, 73, 192, 53],
    'TM_2025': [
        110.97229571984437, 78.61645569620254, 64.0510650887574,
        60.736447368421054, 72.21077777777778, 79.01486772486773,
        80.86041095890411, 62.768229166666664, 19.90566037735849
    ],
    'PROD_PROMOCIONADO': ['AÇÃO DIA DAS MÃES'] * 9,
    'COMPOSICAO_PROD': ['DRINK DONA ISAURA GRÁTIS'] * 9,
    'QUANTIDADE': [0, 0, 0, 1, 0, 0, 0, 0, 0],
    'VALOR_VENDA_PROD': [0, 0, 0, 0, 0, 0, 0, 0, 0],
...
with st.sidebar:
    st.markdown(
        ...
        f"color:{ACCENT};margin-bottom:1rem;text-transform:uppercase;letter-spacing:.05em;'>🎯 Filtros</div>",
        unsafe_allow_html=True
    )
    ...
    st.markdown(
        ...
        f"Dia das Mães Buteco 2026<br>Dashboard de Performance</div>",
        unsafe_allow_html=True
    )
...
with col1:
    ...
    if not df_filtrado.empty:
        ...
        st.markdown('<div style="text-align:center;padding:2rem;color:#89CFF0;">Nenhuma loja com dados.</div>', unsafe_allow_html=True)
...
with col2:
    ...
    if not df_comparavel.empty:
        ...
        cores = [ACCENT if x >= 0 else "#003366" for x in df_comp['VARIACAO_%']]
        y_font_size = 11
...
def build_ranking_iframe(datasets):
    ...
    for title, rows in datasets:
        ...
        for rank, loja, valor_str, label, pct in rows:
            ...
            num_color = medal_colors.get(rank, ACCENT)
            ...
            <div style="
                ...
                background:rgba(0,51,102,{bg_opacity});
                border:1px solid rgba(0,51,102,{border_opacity});
            ...
            ">
                ...
                <div style="text-align:right; white-space:nowrap;">
                    <div style="font-family:'Barlow Condensed',sans-serif; font-size:1.05rem;
                        font-weight:800; color:{ACCENT};">{valor_str}</div>
        ...
        <div style="flex:1; background:linear-gradient(145deg,{BG_CARD} 0%,{BG_DARK} 100%);
            border:1px solid rgba(0,51,102,.2); border-radius:18px; padding:18px 16px;
            box-shadow:0 8px 32px rgba(0,0,0,.4);">
            <div style="font-family:'Barlow Condensed',sans-serif; font-size:.95rem; font-weight:800;
                color:{ACCENT}; text-transform:uppercase; letter-spacing:.08em;
                margin-bottom:14px; padding-bottom:10px;
                border-bottom:1px solid rgba(0,51,102,.2);">{title}</div>
            {items}
        </div>"""
...
<div style="margin-top: 2rem;">
    <div class="info-card">
        <div class="info-title">
            📌 AVISOS IMPORTANTES — METODOLOGIA
        </div>
        <div class="info-text">
            <ul class="info-list">
                ...
                <li><strong style="color: #89CFF0;">Olegário foi a única loja que vendeu a dose dupla da caipirinha de limão em dose dupla.</strong></li>
                <li><strong style="color: #89CFF0;">* As lojas sem faturamento em 2025 ou 2026 não são consideradas nos cálculos de variação e comparativos.</strong></li>
            </ul>
        </div>
    </div>
</div>
...
st.markdown(
    ...
    f"Dashboard Buteco • Desenvolvido com Streamlit + Plotly • "
